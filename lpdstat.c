 ...
 *
 *
 * Hacked by Seb to give lpd queue status.
 */
...
static int	lpd_status(const char *hostname, int port, const char *printer,
			   const char *user, int reserve, int timeout);
static void	lpd_timeout(int sig);
static int	lpd_write(int lpd_fd, char *buffer, int length);
static int	lpd_read(int lpd_fd, char *buffer, int length, int timeout);
...
/*
 * 'lpd_status()' - Query a queue using the Line Printer Daemon protocol.
 */

static int				/* O - Zero on success, non-zero on failure */
lpd_status(const char *hostname,		/* I - Host to connect to */
	   ...
	   int        timeout)		/* I - Timeout... */
{
 ...
 /*
  * Loop forever trying to get status...
  */
  ...
  for (;;) /* FOREVER */
  {
    ...
    */
    ...
    status = lpd_command(fd, timeout, "\003%s\n", printer);

    fprintf (stderr, "INFO: Read %d bytes (timeout was %d)",
	     lpd_read (fd, control, 10240, timeout), timeout);

    fprintf (stderr, "INFO: Status (short): '%s'\n", control);
     ...
     char *argv[])			/* I - Command-line arguments */
{
  ...
  * Query the queue...
  */
  status = lpd_status (hostname, port, resource, username, reserve, timeout);
...
static int				/* O - Number of bytes read or -1 on error */
lpd_read(int  lpd_fd,			/* I - LPD socket */
	 char *buffer,			/* I - Buffer to read into */
	 int  length,			/* I - Number of bytes available in buffer */
	 int timeout)
{
  int	bytes,				/* Number of bytes read */
	total;				/* Total number of bytes read */


  alarm (timeout);
  total = 0;
  while ((bytes = recv(lpd_fd, buffer, length - total, MSG_WAITALL)) > 0)
  {
    fprintf (stderr, "INFO: received %d bytes into buffer\n", bytes);
    total  += bytes;
    buffer += bytes;
