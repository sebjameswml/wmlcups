/*
 * Files common both to cups-lpd.c and rawprint.c (to be called
 * cups-raw.c later). This code is simply #included in the cups-lpd.c
 * and rawprint.c/cups-raw.c file.
 */
...
#ifdef CHECK_CUPSD_MAXJOBS_FEATURE
#include <sys/ipc.h>
#include <sys/shm.h>
#include "cupsd_numjobs.h"
int check_cupsd_for_maxjobs (http_t * connection);
#endif

#ifdef NEED_PRINT_FILE_FUNCTION
int					/* O - Job ID */
...
check_cupsd_for_maxjobs (http_t * connection)
{
  ...
  /*syslog (LOG_INFO, "MyNumJobs(%d) - MyMaxJobs(%d) = %d", MyNumJobs, MyMaxJobs, MyNumJobs - MyMaxJobs);*/
/*
  if (MyNumJobs - MyMaxJobs == 0) {
     * AT MaxJobs, that means we can't print another, but it may be
     * that there are printed files that can be cleaned out. We _have_
     * to be able to call CleanJobs() here before we decided if we can
     * spool to cupsd - ah, but I do this in get_maxjobs in ipp.c in
     * cups
    cupsd_jobs_ok = 0;
  }
*/
...
#endif /* CHECK_CUPSD_MAXJOBS_FEATURE */
...
#endif /* NEED_PRINT_FILE_FUNCTION */
